...
//Rare exceptions to indicate a bug in the _rodas_reinel_project1 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_rodas_reinel_project1 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_rodas_reinel_project1]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.rodas_reinel_project1.module"
